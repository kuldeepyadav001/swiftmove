...
function Row({ label, sub, amount, highlight }) {
  return (
    <div
      className={`flex items-center justify-between py-2 ${highlight ? "border-t border-slate-200 mt-1 pt-3" : "border-b border-slate-50"}`}
    >
      <div>
        <span
          className={`text-xs ${highlight ? "font-bold text-slate-900" : "text-slate-600"}`}
        >
          {label}
        </span>
      ...
      <span
        className={`text-xs font-semibold ${highlight ? "text-blue-700 text-sm font-extrabold" : "text-slate-800"}`}
      >
        {amount}
      </span>
    </div>
    ...
    return (
      <div className="bg-slate-50 border border-dashed border-slate-200 rounded-xl p-4 text-center">
        <p className="text-xs text-slate-400">
          Enter pickup, drop and select a vehicle to see fare
        </p>
      </div>
    );
        ...
        {!fare.orsApiUsed && (
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
            ...
            <p className="text-xs text-amber-700 leading-relaxed">
              Using estimated distance. Add ORS API key for exact road distance
              and more accurate fares.
            </p>
          </div>
        )}

        {/* Fare breakdown — simple: base fare + distance only */}
        <Row
          label={`Base fare`}
          sub={`first ${fare.includedDistanceKm} km incl.`}
